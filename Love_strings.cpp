    ...
    for (ll i = 2; i * i <= n; ++i) // Improve start by i = 3.
    {
        while (n % i == 0) {
            primes.push_back(i),
                    ...
                    n = i; // Get every prime inside n.  n  i^j  is a new number
        }
    }
...
bool isDistinc(int n) {
    ...
    for (size_t i = 0; i < 3; i++) {
        if (last[i] == last[i + 1]) { // cout<<last[i]<<" "<<last[i+1]<<" ";
            return 1;
        }
    }
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int i = m.length() ; i < ma.length(); i++)
            cout << ma[i];
        cout << endl;
    }
    return 0;
